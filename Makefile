...
PACK             := grafana
PACKDIR          := sdk
PROJECT          := github.com/crasiak/pulumi-${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/crasiak/pulumi-grafana,${REPOSITORY},g' provider/go.mod; \
...
tfgen::
	cd provider && go install -ldflags "-X github.com/crasiak/pulumi-${PACK}/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${TFGEN}

provider:: generate_schema
	cd provider && go generate cmd/${PROVIDER}/main.go
	cd provider && go install -ldflags "-X github.com/crasiak/pulumi-${PACK}/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${PROVIDER}
